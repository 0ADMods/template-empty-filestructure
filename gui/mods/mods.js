...
var g_spamMonitor = {};
var g_timestamp = Engine.ConfigDB_GetValue("user", "mods.chattimestamp") == "true";
var g_mapSizes = {};
...
function modsConnect()
{
	Engine.ConnectXmppClient();
}

function modsDisconnect()
{
...
function updateModList()
{
	...
	for each (var g in modList)
	{
		if(!filterGame(g))
		{
			...
			list_name.push(name);
			list_ip.push(g.ip);
			list_mapName.push(g.niceModName);
...
function updateGameSelection()
{
	...
	// Load map data
	if (g_ModList[g].mapType == "random" && g_ModList[g].mapName == "random")
		mapData = {"settings": {"Description": "A randomly selected map."}};
	else if (g_ModList[g].mapType == "random" && Engine.FileExists(g_ModList[g].mapName + ".json"))
		mapData = parseJSONData(g_ModList[g].mapName + ".json");
	else if (Engine.FileExists(g_ModList[g].mapName + ".xml"))
		mapData = Engine.LoadMapSettings(g_ModList[g].mapName + ".xml");
	else
		// Warn the player if we can't find the map. 
		warn("Map '" + g_ModList[g].mapName + "' not found locally.");
	...
	Engine.GetGUIObjectByName("sgModName").caption = g_ModList[g].niceModName;
	Engine.GetGUIObjectByName("sgNbPlayers").caption = g_ModList[g].nbp + "/" + g_ModList[g].tnbp;
	Engine.GetGUIObjectByName("sgPlayersNames").caption = g_ModList[g].players;
	Engine.GetGUIObjectByName("sgMapSize").caption = g_ModList[g].mapSize.split("(")[0];
	Engine.GetGUIObjectByName("sgMapType").caption = toTitleCase(g_ModList[g].mapType);
	...
	Engine.GetGUIObjectByName("sgModDescription").caption = mapDescription;
...
function joinSelectedGame()
{
	...
	if (gamesBox.selected >= 0)
	{
		...
		// Open Mods connection window with join option.
		Engine.PushGuiPage("page_gamesetup_mp.xml", { modsGameType: "join", name: sname, ip: sip, rating: g_userRating });
	}
}
...
function hostGame()
{
	// Open Mods connection window with host option.
	Engine.PushGuiPage("page_gamesetup_mp.xml", { modsGameType: "host", name: g_Name, rating: g_userRating });
}

var TAUCETI = (function() {/*<--scope for preventing conflicts with other AIs or API than common-api:
     that create classes that are called the same.*/
var ai = {};//<-- the scope, will be copied to the global TAUCETI on return.

// "local" global variables for stuffs that will need a unique ID
// Note that since order of loading is alphabetic, this means this file must go before any other file using theai.
ai.playerGlobals = [];
...
/**
 * Special for this type of AI.
 */
ai.TauCetiBot = function TauCetiBot(settings) {
        
        //relay to super class:
        API3.BaseAI.call(this, settings);

	//<------- CONSTRUCTOR ------------------->//
        
        //<------- LOCAL PRIVATE VARIABLES .------>//
        var privateLocalVar_functionExecutedCount; /*only accessible from within this constructor,
                also from within functions defined within this constructor!*/
        //this.getBarkCount = function() {<-- this would be separate for every class instantiation.
        //    return "Dog " + this.dogName + " has barked " + barkCount + " times.";
        //}
	...
	/*NOTE: the ai.Config looks strange because the ai = {} was just created some 
	lines above. The key here is that we are currently defining a function aka class, it 
	will be called  once new ai.TauCetiBot(settings) is called and at that point we 
	will have ai.Config class defined already. */
...
//EXTEND THE CLASS THE FIRST TIME:
ai.TauCetiBot.prototype = new API3.BaseAI();

//EXTEND THE CLASS:
ai.TauCetiBot.prototype.CustomInit = function(/*this looks like a relict ->*/gameState, sharedScript) {
	...
	for (var i in this.priorities)
		this.queues[i] = new ai.Queue();

	this.queueManager = new ai.QueueManager(this.config, this.queues, this.priorities);

	this.HQ = new ai.HQ(this.config);//<-- in my opinion a HQ needs a building underneath. Why not use myKeyEntities CivCenter?
	gameState.Config = this.config;

	ai.playerGlobals[PlayerID] = {};
	ai.playerGlobals[PlayerID].uniqueIDBOPlans = 0;	// training/building/research plans
	ai.playerGlobals[PlayerID].uniqueIDBases = 1;	// base manager ID. Starts at one because "0" means "no base" on the map
	ai.playerGlobals[PlayerID].uniqueIDTPlans = 1;	// transport plans. starts at 1 because 0 might be used as none.	
	ai.playerGlobals[PlayerID].uniqueIDArmy = 0;

	this.HQ.init(gameState,this.queues);
	ai.debug ("Initialized with the difficulty " + this.config.difficulty);
...
ai.TauCetiBot.prototype.OnUpdate = function(sharedScript) {
	...
	if ((this.turn + this.player) % 8 == 5) {
		...
		Engine.ProfileStart("TauCeti bot (player " + this.player +")");
		...
		if (this.pathInfo !== undefined)
		{
			...
			if (this.pathInfo.angle > Math.PI*2.0)
			{
				...
				{
					ai.debug ("Assuming this is a water map");
		...
		{
			var plan = new ai.ResearchPlan(this.gameState, townPhase, true);
			plan.lastIsGo = false;
			...
			plan.onGo = function (gameState) { gameState.ai.HQ.econState = "townPhasing"; ai.debug ("Trying to reach TownPhase"); };
			plan.onNotGo = function (gameState) { gameState.ai.HQ.econState = "growth"; };

			this.queues.majorTech.addItem(plan);
			ai.debug ("Planning Town Phase");
			...
			ai.debug ("Trying to reach city phase");
			this.queues.majorTech.addItem(new ai.ResearchPlan(this.gameState, cityPhase));
		}
		// defcon cooldown
		if (this.defcon < 5 && this.gameState.timeSinceDefconChange() > 20000)
		{
			this.defcon++;
			ai.debug ("updefconing to " +this.defcon);
...
ai.TauCetiBot.prototype.initPersonality = function(gameState)
{
...
/*ai.TauCetiBot.prototype.Deserialize = function(data, sharedScript)
{
};

// Override the default serializer
TauCetiBot.prototype.Serialize = function()
{
	return {};
};*/

// For the moment we just use the debugging flag and the debugging function from the API.
// Maybe it will make sense in the future to separate theai.
ai.DebugEnabled = function()
{
	return API3.DebugEnabled;
}

ai.debug = function(output)
{
	API3.debug(output);
}

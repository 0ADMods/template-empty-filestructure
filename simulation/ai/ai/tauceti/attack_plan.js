...
ai.CityAttack = function CityAttack(gameState, HQ, Config, uniqueID, targetEnemy, type , targetFinder) {
	...
	ai.debug ("Target (" + PlayerID +") = " +this.targetPlayer);
...
ai.CityAttack.prototype.getName = function(){
	return this.name;
};
ai.CityAttack.prototype.getType = function(){
	return this.type;
};
...
ai.CityAttack.prototype.canStart = function(gameState){	
	for (var unitCat in this.unitStat) {
		var Unit = this.unitStat[unitCat];
		if (this.unit[unitCat].length < Unit["minSize"])
		{
			ai.debug(unitCat + " doesn't have enough units : " + this.unit[unitCat].length);
			return false;
		}
	}
	return true;
...
ai.CityAttack.prototype.isStarted = function(){
	if ((this.state !== "unexecuted"))
		ai.debug ("Attack plan already started");
	return !(this.state == "unexecuted");
};

ai.CityAttack.prototype.isPaused = function(){
	return this.paused;
};
ai.CityAttack.prototype.setPaused = function(gameState, boolValue){
	if (!this.paused && boolValue === true) {
		this.pausingStart = gameState.getTimeElapsed();
		this.paused = true;
		ai.debug ("Pausing attack plan " +this.name);
	} else if (this.paused && boolValue === false) {
		this.totalPausingTime += gameState.getTimeElapsed() - this.pausingStart;
		this.paused = false;
		ai.debug ("Unpausing attack plan " +this.name);
	}
};
ai.CityAttack.prototype.mustStart = function(gameState){
...
ai.CityAttack.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue) {
	if (!this.isStarted())
	{
		ai.debug ("Adding a build order for " + name);
...
ai.CityAttack.prototype.updatePreparation = function(gameState, HQ,events) {
	var self = this;
	...
	if (this.path == undefined || this.target == undefined || this.path === "toBeContinued") {
		...
		if (this.path === undefined) {
			...
			{
				ai.debug ("This is actually a water map.");
				gameState.ai.HQ.waterMap = true;
				return 0;
			}
			ai.debug ("We need a ship.");
			this.needsShip = true;
			this.pathWidth = 3;
			this.pathSampling = 3;
	...
	if ((gameState.getPopulationMax() - gameState.getPopulation() < 10) && this.canStart())
	{
		...
		if (queued + inTraining + this.buildOrder[0][2].length <= this.buildOrder[0][3]["targetSize"]) {
			...
			if (this.buildOrder[0][0] < 1 && queue.length() <= 5) {
				...
				//ai.debug ("tried " + uneval(this.buildOrder[0][1]) +", and " + template);
				...
				if (template === undefined) {
					...
					if (gameState.getTemplate(template).hasClass("CitizenSoldier"))
						queue.addItem( new ai.TrainingPlan(gameState,template, { "role" : "worker", "plan" : this.name, "special" : specialData, "base" : 1 }, this.buildOrder[0][3]["batchSize"],max ) );
					else
						queue.addItem( new ai.TrainingPlan(gameState,template, { "role" : "attack", "plan" : this.name, "special" : specialData, "base" : 1 }, this.buildOrder[0][3]["batchSize"],max ) );
				}
			}
		}
		/*
		if (!this.startedPathing && this.path === undefined) {
			...
			if (targets.length) {
				...
				while (!this.targetPos){
					...
					count++;
					if (count > 1000){
						ai.debug("No target with a valid position found");
						Engine.ProfileStop();
						Engine.ProfileStop();
						return false;
					}
		...
		*/
		...
		if (this.buildOrder.length === 0) {
			ai.debug ("Ending plan: no build orders");
...
ai.CityAttack.prototype.assignUnits = function(gameState){
	var self = this;
...
ai.CityAttack.prototype.ToRallyPoint = function(gameState,id)
{	
...
ai.CityAttack.prototype.AllToRallyPoint = function(gameState, evenWorkers) {
	var self = this;
	if (evenWorkers) {
...
ai.CityAttack.prototype.defaultTargetFinder = function(gameState, HQ){
...
// tupdate
ai.CityAttack.prototype.raidingTargetFinder = function(gameState, HQ, Target){
	var targets = undefined;
	if (Target == "villager")
	{
...
ai.CityAttack.prototype.StartAttack = function(gameState, HQ){
	...
	if (this.targetPos && this.path !== undefined) {
		...
		ai.debug ("I do not have any target. So I'll just assume I won the game.");
		return false;
	}
	return true;
};
...
ai.CityAttack.prototype.update = function(gameState, HQ, events){
	var self = this;
	...
	if (this.state === "walking") {
		...
		var territoryMap = ai.createTerritoryMap(gameState);
		if ((territoryMap.getOwner(this.position) === this.targetPlayer && attackedNB > 1) || attackedNB > 4) {
			ai.debug ("Attack Plan " +this.type +" " +this.name +" has arrived to destination.");
						...
						if (this.threatList.indexOf(e.msg.attacker) === -1)
						{
							...
							for (var j in enemySoldiers)
							{
								...
								if (enemy.position() === undefined)	// likely garrisoned
									continue;
								if (ai.inRange(enemy.position(), attacker.position(), 1000) && this.threatList.indexOf(enemy.id()) === -1)
	...
	if (this.state === "walking"){
		...
		if (this.lastPosition && API3.SquareVectorDistance(this.position, this.lastPosition) < 20 && this.path.length > 0) {
			...
			{
				ai.debug ("Attack Plan " +this.type +" " +this.name +" has met walls and is not happy.");
				this.state = "arrived";
			} else if (nexttoWalls) {
				// abort plan.
				ai.debug ("Attack Plan " +this.type +" " +this.name +" has met walls and gives up.");
				Engine.ProfileStop();
				return 0;
			}
		}
				...
				&& API3.SquareVectorDistance(this.unitCollection.filter(API3.Filters.byClass("Siege")).getCentrePosition(), this.path[0][0]) >= 650)
			{
				...
				} else
				{
					...
					if (this.path.length === 0) {
						...
						/*
						var plan = new ai.TransportPlan(gameState, this.unitCollection.toIdArray(), this.path[0][0], false);
						this.tpPlanID = plan.ID;
						HQ.navalManager.transportPlans.push(plan);
						ai.debug ("Transporting over sea");
	...
	if (this.state === "arrived"){
		...
		if (this.onArrivalReaction == "proceedOnTargets") {
			...
			enemyCitizens.forEach( function (enemy) {
				if (ai.inRange(enemy.position(), units.getCentrePosition(), 2500) && targetList.indexOf(enemy.id()) === -1)
	...
	if (this.state === "") {
				...
				if (ent.hasClass("Siege")) {
					...
					if (mStruct.length !== 0) {
						...
						else
						{
							...
							//ai.debug ("Siege units attacking a structure from " +mStruct[+rand].owner() + " , " +mStruct[+rand].templateName());
						}
					} else if (API3.SquareVectorDistance(self.targetPos, ent.position()) > 900 ) {
						//ai.debug ("Siege units moving to " + uneval(self.targetPos));
					...
					if (mUnit.length !== 0) {
						...
						//ai.debug ("Units attacking a unit from " +mUnit[+rand].owner() + " , " +mUnit[+rand].templateName());
					} else if (API3.SquareVectorDistance(self.targetPos, ent.position()) > 900 ){
						//ai.debug ("Units moving to " + uneval(self.targetPos));
						ent.move(self.targetPos[0],self.targetPos[1]);
						...
						else
						{
							...
							ent.attack(mStruct[+rand].id());
							//ai.debug ("Units attacking a structure from " +mStruct[+rand].owner() + " , " +mStruct[+rand].templateName());
						}
					}
				}
			}
		...
		{			
			...
			if (targets.length) {
				ai.debug ("Seems like our target has been destroyed. Switching.");
				ai.debug ("Aiming for " + targets);
				// picking a target
				this.targetPos = undefined;
				var count = 0;
...
ai.CityAttack.prototype.totalCountUnits = function(gameState){
	var totalcount = 0;
...
// reset any units
ai.CityAttack.prototype.Abort = function(gameState){	
	this.unitCollection.forEach(function(ent) {

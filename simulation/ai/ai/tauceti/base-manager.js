...
ai.BaseManager = function(Config) {
	this.Config = Config;
	this.farmingFields = false;
	this.ID = ai.playerGlobals[PlayerID].uniqueIDBases++;
...
ai.BaseManager.prototype.init = function(gameState, unconstructed){
...
ai.BaseManager.prototype.assignEntity = function(unit){
...
ai.BaseManager.prototype.setAnchor = function(anchorEntity) {
	if (!anchorEntity.hasClass("Structure") || !anchorEntity.hasTerritoryInfluence())
	{
		warn("Error: TauCeti' base " + this.ID + " has been assigned an anchor building that has no territorial influence. Please report this on the foruai.")
		return false;
	}
...
// affects the HQ map.
ai.BaseManager.prototype.initTerritory = function(HQ, gameState) {
	if (!this.anchor)
		warn ("Error: TauCeti tried to initialize the territory of base " + this.ID + " without assigning it an anchor building first");
...
ai.BaseManager.prototype.initGatheringFunctions = function(HQ, gameState, specTypes) {
	...
	ai.debug ("food" + this.willGather["food"]);
	ai.debug (this.willGather["wood"]);
	ai.debug (this.willGather["stone"]);
	ai.debug (this.willGather["metal"]);
}

ai.BaseManager.prototype.checkEvents = function (gameState, events, queues) {
			...
			evt.metadata[PlayerID]["base"] !== undefined && evt.metadata[PlayerID]["base"] == this.ID)
		{
			...
			if (evt.metadata[PlayerID]["baseAnchor"] && evt.metadata[PlayerID]["baseAnchor"] == true)
			{
				...
				if (ent.hasClass("CivCentre"))
				{
					...
					queues.civilCentre.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_civil_centre", { "base" : this.ID, "baseAnchor" : true }, ent.position()));
				} else {
					// TODO
					queues.civilCentre.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_civil_centre", { "base" : this.ID, "baseAnchor" : true }, ent.position()));
				}
...
ai.BaseManager.prototype.assignResourceToDP = function (gameState, supply, specificDP) {
...
ai.BaseManager.prototype.initializeDropsite = function (gameState, ent, type) {
	...
	if (self.dropsites[ent.id()] === undefined || self.dropsites[ent.id()][type] === undefined) {
		...
		// TODO: get workers on those resources and do something with theai.
	}
...
ai.BaseManager.prototype.scrapDropsite = function (gameState, ent) {
...
ai.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource){
	...
	var obstructions = ai.createObstructionMap(gameState,this.accessIndex,storeHousePlate);
	...
	ai.debug ("for dropsite best is " +best[1] + " at " + gameState.getTimeElapsed());
...
ai.BaseManager.prototype.updateDropsite = function (gameState, ent, type) {
		...
		&& gameState.ai.queues.dropsites.length() === 0 && gameState.countFoundationsByType(gameState.applyCiv("structures/{civ}_storehouse"), true) === 0)
	{
		...
		{
			dropsite[6] = true;
			gameState.ai.queues.dropsites.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_storehouse", { "base" : this.ID }, [medianPositionX,medianPositionY]));
		}
	}
...
// Updates dropsites.
ai.BaseManager.prototype.updateDropsites = function (gameState) {
...
ai.BaseManager.prototype.getDpWorkerCapacity = function (gameState, entID, type, faraway) {
...
ai.BaseManager.prototype.getWorkerCapacity = function (gameState, type, faraway) {
	var count = 0;
	for (var i in this.dropsites)
...
ai.BaseManager.prototype.getResourceLevel = function (gameState, type, searchType, threshold) {
	var count = 0;
	if (searchType == "all")
	{
...
ai.BaseManager.prototype.checkResourceLevels = function (gameState,queues) {
	for (var type in this.willGather)
	{
		...
		if (type == "food")
		{
			if (!this.isFarming && count < 1600 && queues.field.length === 0)
			{
				...
				{
					var plan = new ai.ConstructionPlan(gameState, "structures/{civ}_field", { "base" : this.ID });
			...
			if (this.isFarming)
			{
				...
				if (numQueued < 3)
					if (numFarms < Math.round(this.gatherersByType(gameState, "food").length / (maxGatherers*0.9)))
						queues.field.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_field", { "base" : this.ID }));
			...
			if (wantDropsite)
			{
				...
				if (!pos)
				{
					ai.debug ("Found no right position for a " + type + " dropsite, going into \"noSpot\" mode");
					...
					ai.debug ("planning new dropsite for " + type);
					queues.dropsites.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_storehouse",{ "base" : this.ID }, pos));
				}
			}
		}
	}
...
ai.BaseManager.prototype.getGatherRates = function(gameState, currentRates) {
	for (var i in currentRates)
	{
		...
		currentRates[i] += 0.5*ai.GetTCRessGatherer(gameState,i);
	}
};

ai.BaseManager.prototype.assignRolelessUnits = function(gameState) {
...
ai.BaseManager.prototype.setWorkersIdleByPriority = function(gameState){
	var self = this;
		...
		if (types[types.types[i]] > avgOverdraft + 200 || (types[types.types[i]] > avgOverdraft && avgOverdraft > 200))
			if (this.gatherersByType(gameState,types.types[i]).length > 0)
			{
				...
				this.gatherersByType(gameState,types.types[i]).forEach( function (ent) { //}){
					if (nb > 0)
					{
						//ai.debug ("Moving " +ent.id() + " from " + types.types[i]);
						nb--;
	...
	//ai.debug (currentRates);
};

// TODO: work on this.
ai.BaseManager.prototype.reassignIdleWorkers = function(gameState) {
...
ai.BaseManager.prototype.workersBySubrole = function(gameState, subrole) {
...
ai.BaseManager.prototype.gatherersByType = function(gameState, type) {
...
ai.BaseManager.prototype.pickBuilders = function(gameState, workers, number) {
	// TODO: choose better.
...
ai.BaseManager.prototype.assignToFoundations = function(gameState, noRepair) {
...
ai.BaseManager.prototype.update = function(gameState, queues, events) {
	...
	{
		var terrMap = ai.createTerritoryMap(gameState);
...
//	}
	...
	this.workers.forEach(function(ent) {
		if (!ent.getMetadata(PlayerID, "worker-object"))
			ent.setMetadata(PlayerID, "worker-object", new ai.Worker(ent));

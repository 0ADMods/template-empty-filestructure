...
ai.HQ = function(Config) {
	...
	this.defenceManager = new ai.Defence(this.Config);
	this.navalManager = new ai.NavalManager();
...
ai.HQ.prototype.init = function(gameState, queues){
	...
	if (hasCC)
	{
		...
		this.baseManagers[1] = new ai.BaseManager(this.Config);
	...
	if (this.baseManagers[1])
	{
		if (gameState.ai.queueManager.getAvailableResources(gameState)["wood"] >= 250)
		{
			...
			if (pos)
			{
				queues.dropsites.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_storehouse",{ "base" : 1 }, pos));
				queues.minorTech.addItem(new ai.ResearchPlan(gameState, "gather_capacity_wheelbarrow"));
			}
		}
	}
...
ai.HQ.prototype.checkEvents = function (gameState, events, queues) {
	...
	for (var i in CreateEvents)
	{
		...
		if (evt && evt.entity)
		{
			...
			if (ent.isOwn(PlayerID) && ent.getMetadata(PlayerID, "base") === -1)
			{
				...
				var bID = ai.playerGlobals[PlayerID].uniqueIDBases;
				this.baseManagers[bID] = new ai.BaseManager(this.Config);
...
ai.HQ.prototype.OnTownPhase = function(gameState)
{
...
ai.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// Get some data.
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		ent.trainingQueue().forEach(function(item) {
			if (iteai.metadata && iteai.metadata.role && iteai.metadata.role == "worker" && iteai.metadata.plan == undefined)
				numWorkers += iteai.count;
			numInTraining += iteai.count;
		});
	});
	...
	*/
	...
	if (template === gameState.applyCiv("units/{civ}_support_female_citizen"))
		queues.villager.addItem(new ai.TrainingPlan(gameState, template, { "role" : "worker", "base" : 0 }, size, size ));
	else
		queues.citizenSoldier.addItem(new ai.TrainingPlan(gameState, template, { "role" : "worker", "base" : 0 }, size, size));
};

// picks the best template based on parameters and classes
ai.HQ.prototype.findBestTrainableUnit = function(gameState, classes, parameters) {
	...
	units.sort(function(a, b) {// }) {
		...
		for (var i in parameters) {
			...
			if (param[0] == "strength") {
				aTopParam += ai.getMaxStrength(a[1]) * param[1];
				bTopParam += ai.getMaxStrength(b[1]) * param[1];
			}
			if (param[0] == "siegeStrength") {
				aTopParam += ai.getMaxStrength(a[1], "Structure") * param[1];
				bTopParam += ai.getMaxStrength(b[1], "Structure") * param[1];
			}
			if (param[0] == "speed") {
...
ai.HQ.prototype.tryResearchTechs = function(gameState, queues) {
	if (queues.minorTech.length() === 0)
	{
		...
		queues.minorTech.addItem(new ai.ResearchPlan(gameState, possibilities[p][0]));
	}
}
...
ai.HQ.prototype.switchWorkerBase = function(gameState, worker, type) {
	var bestBase = 0;
	var bestBaseState = -1;
...
ai.HQ.prototype.bulkPickWorkers = function(gameState, newBaseID, number) {
	var accessIndex = this.baseManagers[newBaseID].accessIndex;
	if (!accessIndex)
		return false;
	// sorting bases by whether they are on the same accessindex or not.
	var baseBest = ai.AssocArraytoArray(this.baseManagers).sort(function (a,b) {
...
ai.HQ.prototype.GetCurrentGatherRates = function(gameState) {
	var self = this;
 ...
 */
ai.HQ.prototype.pickMostNeededResources = function(gameState) {
	var self = this;
...
// TODO: rehabilitate.
ai.HQ.prototype.buildNewCC= function(gameState, queues) {
	...
	if (gameState.defcon() > 2)
		for (var i = numCCs; i < 1; i++) {
			...
			queues.civilCentre.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_civil_centre"));
		}
...
ai.HQ.prototype.findBestEcoCCLocation = function(gameState, resource){
	...
	var territory = ai.createTerritoryMap(gameState);
	
	var obstructions = ai.createObstructionMap(gameState, 0);
	...
	for (var j = 0; j < friendlyTiles.length; ++j)
	{
		...
		// TODO: maybe whenever I get around to implement multi-base support (details below, requires being part of the teaai. If you're not, ask wraitii directly by PM).
	...
	ai.debug ("Best for value " + best[1] + " at " + uneval([x,z]));

	return [x,z];
};

ai.HQ.prototype.buildTemple = function(gameState, queues){
	if (gameState.currentPhase() >= 2 ) {
		if (queues.economicBuilding.countQueuedUnits() === 0 &&
			...
			queues.economicBuilding.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_temple", { "base" : 1 }));
		}
	}
};

ai.HQ.prototype.buildMarket = function(gameState, queues){
	if (gameState.getPopulation() > this.Config.Economy.popForMarket && gameState.currentPhase() >= 2 ) {
		if (queues.economicBuilding.countQueuedUnitsWithClass("BarterMarket") === 0 &&
			...
			queues.economicBuilding.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_market", { "base" : 1 }));
		}
	}
};

// Build a farmstead to go to town phase faster and prepare for research. Only really active on higher diff mode.
ai.HQ.prototype.buildFarmstead = function(gameState, queues){
	if (gameState.getPopulation() > this.Config.Economy.popForFarmstead
		...
		if (queues.economicBuilding.countQueuedUnitsWithClass("DropsiteFood") === 0 &&
			...
			//only ever build one storehouse/CC/market at a time
			queues.economicBuilding.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_farmstead", { "base" : 1 }));
			// add the farming plough to the research we want.
			queues.minorTech.addItem(new ai.ResearchPlan(gameState, "gather_farming_plows"));
		}
	}
};

// TODO: generic this, probably per-base
ai.HQ.prototype.buildDock = function(gameState, queues){
	...
	if (gameState.getTimeElapsed() > this.dockStartTime) {
		if (queues.economicBuilding.countQueuedUnitsWithClass("NavalMarket") === 0 &&
			...
			if (tp !== "")
			{
				...
				queues.economicBuilding.addItem(new ai.ConstructionPlan(gameState, tp, { "base" : 1, "sea" : remaining[0] }));
			}
		}
	}
};
...
ai.HQ.prototype.tryBartering = function(gameState) {
	...
	for (var j in gameState.ai.queues)
	{
		...
		for each (var ress in elemCost.types)
		{
			...
			if (bestToBarter !== "")
			{
				...
				ai.debug ("Snipe bartered " + sell +" for " + buy + ", value 100");
				return true;
			}
		}
	}
		...
		for each (var buy in needs.types) {
			if (buy != sell && needs[sell] <= 0 && available[sell] > 500) {	// if we don't need it and have a buffer
				if (needs[buy] > rates[buy]*80) {	// if we need that other resource terribly.
					markets[0].barter(buy,sell,100);
					ai.debug ("Gross bartered " +sell +" for " + buy + ", value 100");
					return true;
				}
			}
		}
...
ai.HQ.prototype.buildMoreHouses = function(gameState,queues) {
	...
	if (gameState.getPopulationLimit() < gameState.getPopulationMax()) {
		...
		{
			var plan = new ai.ConstructionPlan(gameState, "structures/{civ}_house", { "base" : 1 });
...
ai.HQ.prototype.checkBasesRessLevel = function(gameState,queues) {
			...
			|| capacity[type] < gameState.getOwnUnits().filter(API3.Filters.and(API3.Filters.byMetadata(PlayerID, "subrole", "gatherer"), API3.Filters.byMetadata(PlayerID, "gather-type", type))).length * 1.05)
		{
			...
			if (gameState.countFoundationsByType(gameState.applyCiv("structures/{civ}_civil_centre"), true) === 0 && queues.civilCentre.length() === 0) {
				...
				if (!pos)
				{
					...
					queues.civilCentre.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_civil_centre",{ "base" : -1 }, pos));
				}
			}
		}
	}
...
// TODO: Fortresses are placed randomly atai.
ai.HQ.prototype.buildDefences = function(gameState, queues){
			...
			for (var j in this.baseManagers[i].dropsites)
			{
				...
				if (dpEnt !== undefined && dpEnt.getMetadata(PlayerID, "defenseTower") !== true)
					if (amnts["wood"] || amnts["metal"] || amnts["stone"])
					{
						...
						if (position) {
							queues.defenceBuilding.addItem(new ai.ConstructionPlan(gameState, 'structures/{civ}_defense_tower', { "base" : i }, position));
						}
	...
	if (queues.defenceBuilding.length() < 1 && (gameState.currentPhase() > 2 || gameState.isResearching("phase_city_generic")))
	{
		if (workersNumber >= 80 && gameState.getTimeElapsed() > numFortresses * this.fortressLapseTime + this.fortressStartTime)
		{
			...
			queues.defenceBuilding.addItem(new ai.ConstructionPlan(gameState, this.bFort[0], { "base" : 1 }));
			ai.debug ("Building a fortress");
		}
	}
...
ai.HQ.prototype.buildBlacksmith = function(gameState, queues){
	if (gameState.getTimeElapsed() > this.Config.Military.timeForBlacksmith*1000) {
		if (queues.militaryBuilding.length() === 0 &&
			...
			if (tp.available(gameState))
				queues.militaryBuilding.addItem(new ai.ConstructionPlan(gameState, "structures/{civ}_blacksmith", { "base" : 1 }));
		}
	}
};
...
ai.HQ.prototype.constructTrainingBuildings = function(gameState, queues) {
	...
	if (workersNumber > this.Config.Military.popForBarracks1 || (this.econState == "townPhasing" && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)) {
		if (barrackNb + queues.militaryBuilding.length() < 1) {
			ai.debug ("Trying to build barracks");
			var plan = new ai.ConstructionPlan(gameState, this.bModerate[0], { "base" : 1 });
	...
	if (barrackNb < 2 && workersNumber > this.Config.Military.popForBarracks2)
		if (queues.militaryBuilding.length() < 1)
			queues.militaryBuilding.addItem(new ai.ConstructionPlan(gameState, this.bModerate[0], { "base" : 1 }));
		...
		if (queues.militaryBuilding.length() === 0){
			...
			if (inConst == 0 && this.bAdvanced && this.bAdvanced.length !== 0) {
				...
				if (gameState.countEntitiesAndQueuedByType(gameState.applyCiv(this.bAdvanced[i]), true) < 1){
					queues.militaryBuilding.addItem(new ai.ConstructionPlan(gameState, this.bAdvanced[i], { "base" : 1 }));
				}
			}
		}
	}
...
ai.HQ.prototype.garrisonAllFemales = function(gameState) {
...
ai.HQ.prototype.ungarrisonAll = function(gameState) {
...
ai.HQ.prototype.pausePlan = function(gameState, planName) {
...
ai.HQ.prototype.unpausePlan = function(gameState, planName) {
	for (var attackType in this.upcomingAttacks) {
		for (var i in this.upcomingAttacks[attackType]) {
			var attack = this.upcomingAttacks[attackType][i];
			if (attack.getName() == planName)
...
ai.HQ.prototype.pauseAllPlans = function(gameState) {
	for (var attackType in this.upcomingAttacks) {
		for (var i in this.upcomingAttacks[attackType]) {
			var attack = this.upcomingAttacks[attackType][i];
			attack.setPaused(gameState, true);
...
ai.HQ.prototype.unpauseAllPlans = function(gameState) {
	for (var attackType in this.upcomingAttacks) {
		for (var i in this.upcomingAttacks[attackType]) {
			var attack = this.upcomingAttacks[attackType][i];
			attack.setPaused(gameState, false);
...
ai.HQ.prototype.update = function(gameState, queues, events) {	
	...
	for (var i in this.baseManagers)
	{
		...
		if ( ( (+i + gameState.ai.playedTurn) % (ai.playerGlobals[PlayerID].uniqueIDBases - 1)) === 0)
			...
			for (var i = 0;i < this.upcomingAttacks[attackType].length; ++i) {
				...
				if (!attack.isStarted()) {
					...
					if (updateStep === 1 || attack.isPaused() ) {
						...
						ai.debug ("Military Manager: " +attack.getType() +" plan " +attack.getName() +" aborted.");
						if (updateStep === 3) {
							this.attackPlansEncounteredWater = true;
							ai.debug("No attack path found. Aborting.");
						}
						...
						ai.debug ("Military Manager: Starting " +attack.getType() +" plan " +attack.getName());
		...
		for (var i = 0; i < this.startedAttacks[attackType].length; ++i) {
			...
			if (!attack.isPaused())
			{
				...
				if (remaining == 0 || remaining == undefined) {
					ai.debug ("Military Manager: " +attack.getType() +" plan " +attack.getName() +" is now finished.");
	...
	if (!this.waterMap && !this.attackPlansEncounteredWater)
	{
			...
			&& gameState.getTimeElapsed() > this.attackPlansStartTime && gameState.getTimeElapsed() < 360000)
		{
			...
			{ 
				...
				var AttackPlan = new ai.CityAttack(gameState, this, this.Config, this.TotalAttackNumber, -1, "Rush");
				ai.debug ("Headquarters: Rushing plan " +this.TotalAttackNumber);
		...
		// if we have a barracks, there's no water, we're at age >= 1 and we've decided to attack.
			...
			if (gameState.countEntitiesByType(gameState.applyCiv("structures/{civ}_dock"), true) === 0 && this.waterMap)
			{
				...
				if (gameState.getTimeElapsed() < 12*60000)
					Lalala = new ai.CityAttack(gameState, this, this.Config, this.TotalAttackNumber, -1);
				else if (this.Config.difficulty !== 0)
					Lalala = new ai.CityAttack(gameState, this, this.Config, this.TotalAttackNumber, -1, "superSized");

				if (Lalala.failed)
					this.attackPlansEncounteredWater = true; // hack
				else {
					ai.debug ("Military Manager: Creating the plan " +this.TotalAttackNumber);
					this.TotalAttackNumber++;
	...
	/*
	 ...
	 var Lalala = new ai.CityAttack(gameState, this,this.totalStartedAttackNumber, -1, "harass_raid");
	 if (!Lalala.createSupportPlans(gameState, this, )) {
	 ai.debug ("Military Manager: harrassing plan not a valid option");
	 this.HarassRaiding = false;
	 } else {
	 ai.debug ("Military Manager: Creating the harass raid plan " +this.totalStartedAttackNumber);

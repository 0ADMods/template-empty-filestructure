...
ai.NavalManager.prototype.init = function(gameState, queues) {
	// finished docks
...
ai.NavalManager.prototype.getUnconnectedSeas = function (gameState, region) {
...
ai.NavalManager.prototype.canReach = function (gameState, regionA, regionB) {
	...
	for (var i = 0; i < path.length - 1; ++i)
	{
		if (gameState.ai.accessibility.regionType[path[i]] == "land")
			...
			{
				ai.debug ("cannot reach because of " + path[i+1]);
...
ai.NavalManager.prototype.checkEvents = function (gameState, queues, events) {
...
ai.NavalManager.prototype.addPlan = function(plan) {
...
// which can be useful.
ai.NavalManager.prototype.askForTransport = function(entity, startPos, endPos) {
	this.askedPlans.push([entity, startPos, endPos]);
};

// creates aforementionned plans
ai.NavalManager.prototype.createPlans = function(gameState) {
	var startID = {};
	...
	for (var i in startID)
		...
		{
			var tpPlan = new ai.TransportPlan(gameState, startID[i][k].units, startID[i][k].dest, false)
			this.transportPlans.push (tpPlan);
		}
};

// TODO: work on this.
ai.NavalManager.prototype.maintainFleet = function(gameState, queues, events) {
	...
	for (var i = 0; i < this.seaShips.length; ++i)
	{
			...
			&& tpNb + queues.ships.length() === 0 && gameState.getTemplate(gameState.applyCiv("units/{civ}_ship_bireme")).available(gameState))
		{
			...
			queues.ships.addItem(new ai.TrainingPlan(gameState, "units/{civ}_ship_bireme", { "sea" : i }, 1, 1 ));
		}
	}
};

// bumps up the number of ships we want if we need more.
ai.NavalManager.prototype.checkLevels = function(gameState, queues) {
...
ai.NavalManager.prototype.assignToPlans = function(gameState, queues, events) {
	for (var i = 0; i < this.transportPlans.length; ++i)
	{
		var plan = this.transportPlans[i];
		if (plan.needTpShips())
		{
			...
			if (zone)
			{
				for each (var ship in this.seaTpShips[zone]._entities)
				{
					...
					{
						ai.debug ("Assigning ship " + ship.id() + " to plan" + plan.ID);
...
ai.NavalManager.prototype.checkActivePlan = function(ID) {
	for (var i = 0; i < this.transportPlans.length; ++i)
...
ai.NavalManager.prototype.update = function(gameState, queues, events) {

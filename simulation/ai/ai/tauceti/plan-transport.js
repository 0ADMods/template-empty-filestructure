...
ai.TransportPlan = function(gameState, units, destination, allAtOnce, escortSize, onlyIfOK) {
	var self = this;

	this.ID = ai.playerGlobals[PlayerID].uniqueIDTPlans++;
	...
	this.units = ai.EntityCollectionFromIds(gameState, unitsID);
	...
	ai.debug ("Starting a new plan with ID " +  this.ID + " to " + destination);
	ai.debug ("units are " + uneval (units));
...
ai.TransportPlan.prototype.countFreeSlots = function(onlyTrulyFree)
{
	var slots = 0;
...
ai.TransportPlan.prototype.assignShip = function(gameState, ship)
{
	ship.setMetadata(PlayerID,"tpplan", this.ID);
}

ai.TransportPlan.prototype.releaseAll = function(gameState)
{
...
ai.TransportPlan.prototype.releaseAllShips = function(gameState)
{
	this.ships.forEach(function (ent) { ent.setMetadata(PlayerID,"tpplan", undefined) });
}

ai.TransportPlan.prototype.needTpShips = function()
{
...
ai.TransportPlan.prototype.needEscortShips = function()
{
...
ai.TransportPlan.prototype.neededShipsZone = function()
{
	if (!this.seaZone)
		return false;
	return this.seaZone;
 ...
 */
ai.TransportPlan.prototype.carryOn = function(gameState, navalManager)
{
	if (this.state === "unstarted")
	{
		...
		// we have indexes. If there is more than 1, we'll try and regroup theai.
		...
		ai.debug ("plan " +  this.ID + " from " + this.startIndex);
	...
	if (this.state === "waitingForBoarding")
	{
		...
		if (!this.path)
		{
			...
			ai.debug ("Plan " + this.ID + " over seazone " + this.seaZone);
		}
		...
		if (!this.boardingSpot)
		{
			...
			ai.debug ("Plan " + this.ID + " new boarding spot is  " + this.boardingSpot);
		}
	...
	if (this.state === "moving")
	{
		if (!this.unboardingSpot)
		{
			...
			if (this.path.length >= 3)
			{
				...
				ai.debug ("too short at " +uneval(this.path));
	...
	if (this.state === "unboarding")
	{
		...
		if (API3.SquareVectorDistance(this.ships.getCentrePosition(),this.unboardingSpot) > 400)
		{
			...
			{
				ai.debug ("plan " +  this.ID + " going back for more");
				// basically reset.
				delete this.boardingSpot;
			...
			ai.debug ("plan " +  this.ID + " is finished");
			return false;
		}
	}

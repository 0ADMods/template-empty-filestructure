...
ai.QueueManager = function(Config, queues, priorities) {
	this.Config = Config;
	this.queues = queues;
...
ai.QueueManager.prototype.getAvailableResources = function(gameState, noAccounts) {
...
ai.QueueManager.prototype.getTotalAccountedResources = function(gameState) {
...
ai.QueueManager.prototype.currentNeeds = function(gameState) {
...
ai.QueueManager.prototype.futureNeeds = function(gameState) {
...
// TODO: many things.
ai.QueueManager.prototype.wantedGatherRates = function(gameState, shortTerm) {
	// global rates
...
/*ai.QueueManager.prototype.logNeeds = function(gameState) {
 if (!this.totor)
 {
 this.totor = [];
 this.currentGathR = [];
...
ai.QueueManager.prototype.printQueues = function(gameState){
	ai.debug("QUEUES");
	...
	{
		ai.debug(p + ": " + uneval(this.accounts[p]));
	}
	ai.debug("Needed Resources:" + uneval(this.futureNeeds(gameState,false)));
	ai.debug ("Wanted Gather Rates:" + uneval(this.wantedGatherRates(gameState)));
	ai.debug ("Current Resources:" + uneval(gameState.getResources()));
	ai.debug ("Available Resources:" + uneval(this.getAvailableResources(gameState)));
};

// nice readable HTML version.
ai.QueueManager.prototype.HTMLprintQueues = function(gameState){
	if (!ai.DebugEnabled())
		return;
	var strToSend = [];
	strToSend.push("<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01//EN\"> <html> <head> <title>TauCeti Queue Manager</title> <link rel=\"stylesheet\" href=\"table.css\">  </head> <body> <table> <caption>TauCeti Build Order</caption> ");
	for (var i in this.queues){
		strToSend.push("<tr>");
...
ai.QueueManager.prototype.clear = function(){
	this.curItemQueue = [];
	for (var i in this.queues)
		this.queues[i].empty();
};

ai.QueueManager.prototype.update = function(gameState) {
	var self = this;
	...
	for (var i in this.queueArrays)
	{
		...
		if (queue.length() > 0 && !queue.paused)
		{
			...
			if (total.canAfford(iteai.getCost()))
			{
				if (iteai.canStart(gameState))
				{
					this.accounts[name].subtract(iteai.getCost());
...
ai.QueueManager.prototype.pauseQueue = function(queue, scrapAccounts) {
...
ai.QueueManager.prototype.unpauseQueue = function(queue) {
	if (this.queues[queue])
		this.queues[queue].paused = false;
}

ai.QueueManager.prototype.pauseAll = function(scrapAccounts, but) {
	for (var p in this.queues)
		if (p != but)
		{
			if (scrapAccounts)
...
ai.QueueManager.prototype.unpauseAll = function(but) {
	for (var p in this.queues)
		if (p != but)
...
ai.QueueManager.prototype.addQueue = function(queueName, priority) {
	if (this.queues[queueName] == undefined) {
		this.queues[queueName] = new ai.Queue();
...
ai.QueueManager.prototype.removeQueue = function(queueName) {
	if (this.queues[queueName] !== undefined) {
...
ai.QueueManager.prototype.changePriority = function(queueName, newPriority) {
	var self = this;
	if (this.queues[queueName] !== undefined)

...
ai.ConstructionPlan = function(gameState, type, metadata, position) {
...
ai.ConstructionPlan.prototype = Object.create(ai.QueuePlan.prototype);

// checks other than resource ones.
// TODO: change this.
// TODO: if there are specific requirements here, maybe try to do them?
ai.ConstructionPlan.prototype.canStart = function(gameState) {
...
ai.ConstructionPlan.prototype.start = function(gameState) {
	...
	if (!pos){
		...
		ai.debug("No room to place " + this.type);
		return;
	}
	if (this.template.hasClass("Naval"))
		ai.debug (pos);
...
ai.ConstructionPlan.prototype.findGoodPosition = function(gameState) {
	...
	var obstructionMap = ai.createObstructionMap(gameState,0, template);
	...
	if (this.position) {
		...
		gameState.getOwnStructures().forEach(function(ent) {
			...
			if (template.hasClass("Field")) {
				...
				// move farmsteads away to make rooai.

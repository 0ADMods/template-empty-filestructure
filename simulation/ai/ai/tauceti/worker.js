...
ai.Worker.prototype.update = function(baseManager, gameState) {
		...
		if (this.ent.isIdle()) {
			...
			if (gameState.getTimeElapsed() - this.startApproachingResourceTime > this.maxApproachTime)
			{
					...
					&& this.ent.unitAIOrderData()[0]["target"])
				{
					...
					ai.debug ("here " + this.startApproachingResourceAmount + "," + ent.resourceSupplyAmount());
					if (ent && this.startApproachingResourceAmount == ent.resourceSupplyAmount() && this.startEnt == ent.id()) {
						ai.debug (ent.toString() + " is inaccessible");
			...
			// we're gathering. Let's check that it's not a resource we'd rather not gather froai.
			...
			/*if (gameState.getTimeElapsed() - this.startApproachingResourceTime > this.maxApproachTime) {
				...
				if (ent && !ent.isHurt()) {
					// if someone gathers from it, it's only that the pathfinder sucks.
					ai.debug (ent.toString() + " is inaccessible from Combat");
					ent.setMetadata(PlayerID, "inaccessible", true);
					this.ent.flee(ent);
					this.ent.setMetadata(PlayerID, "subrole", "idle");
					this.gatheringFrom = undefined;
...
ai.Worker.prototype.checkUnsatisfactoryResource = function(gameState) {
...
ai.Worker.prototype.startGathering = function(baseManager, gameState) {
	...
	if (wantedDropsite === 0)
	{
		...
		ai.debug("No proper dropsite found for " + resource + ", waiting.");
		return;
	} else
...
ai.Worker.prototype.pickResourceNearDropsite = function(gameState, resource, dropsiteID, forceFaraway)
{
	// get the entity.
	...
	resources.forEach(function(supply) {
		...
		if (ai.IsSupplyFull(gameState, supply) === true)
			return;
...
/*		if (!gameState.ai.accessibility.pathAvailable(gameState, ent.position(), supply.position())) {
		...
		// some simple check for chickens: if they're in a square that's inaccessible, we won't gather from theai.
		...
		var territoryOwner = ai.createTerritoryMap(gameState).getOwner(supply.position());
	...
	if (!wantedSupply)
	{
		...
		//ai.debug("Found a proper dropsite for " + resource + " but apparently no resources are available.");
		return false;
	}
	...
	ai.AddTCGatherer(gameState,wantedSupply.id());
...
ai.Worker.prototype.returnResources = function(gameState){
	if (!this.ent.resourceCarrying() || this.ent.resourceCarrying().length === 0){
	...
	if (!closestDropsite){
		ai.debug("No dropsite found to deposit " + resource);
		return false;
	}
...
ai.Worker.prototype.startHunting = function(gameState, baseManager){
	var ent = this.ent;
	...
	if (resources.length === 0){
		ai.debug("No food found to hunt!");
		return;
	}
	...
	if (nearestSupply) {
		...
		if (!nearestDropsite)
		{
			...
			ai.debug ("No dropsite for hunting food");
			return;
		}
		...
		baseManager.isHunting = false;
		ent.setMetadata(PlayerID, "role", undefined);
		ai.debug("No food found for hunting! (2)");
	}
};

ai.Worker.prototype.getResourceType = function(type){
...
ai.Worker.prototype.getGatherRate = function(gameState) {
	...
	if (this.ent.unitAIOrderData().length && this.ent.unitAIState().split(".")[1] === "GATHER" && this.ent.unitAIOrderData()[0]["target"])
	{
		...
		//ai.debug (+rates[tstring] + " for " + tstring + " for " + this.ent._templateName);
		if (rates[tstring])
...
ai.Worker.prototype.buildAnyField = function(gameState){
	var self = this;
